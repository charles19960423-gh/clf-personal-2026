...
import { knowledgeNodes, systems, videoTopics } from "@/lib/mock-data";
...
function getSystemSymbol(systemKey: (typeof knowledgeNodes)[number]["systemKey"]) {
  return systems.find((system) => system.key === systemKey)?.symbol ?? "";
}

function getSystemName(systemKey: (typeof knowledgeNodes)[number]["systemKey"]) {
  return systems.find((system) => system.key === systemKey)?.name ?? "";
}

function getFirstRelatedNodeTitle(slug: string) {
    ...
    <main className="mx-auto w-full max-w-7xl px-6 py-14 sm:px-10 lg:px-16">
      ...
      <section className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {overviewEntries.map((entry) => (
          <article className="border border-zinc-300 p-6" key={entry.title}>
            <p className="text-sm text-zinc-500">{entry.title}</p>
            <p className="mt-4 text-5xl font-light text-zinc-950">
              {entry.value}
            </p>
            ...
            {recentVideos.map((topic) => (
              <Link className="block py-5" href="/videos" key={topic.id}>
                ...
                <p className="mt-3 text-sm leading-6 text-zinc-600">
                  来源：{getFirstRelatedNodeTitle(topic.relatedNodes[0])}
                </p>
              </Link>
            ))}
          </div>
